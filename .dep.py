...
import os
import re
path=os.path.dirname(os.path.realpath(__file__))+r"\\solver\\"
...
#nx.draw_networkx_nodes(G,node_color='black',pos=pos, node_shape=mpl.markers.MarkerStyle(marker='o', fillstyle='full'),node_size=1)
...
ax.set_xlim([0.9*ax.get_xlim()[0],1.1*ax.get_xlim()[1]])
ax.set_ylim([0.9*ax.get_ylim()[0],1.1*ax.get_ylim()[1]])
...
solver="INSIDEOUT"
rc=subprocess.run([path+"pgraph_solver.exe",solver, path+"input.in", path+"test_out.out", str(num_sol)])
